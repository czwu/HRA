        ...
        <view class="uni-row uni-grow">
          ...
          >
            <view class="item-content-base">
              <text>{{ data1.code }}</text>

              <text
                class="color-text color-text-num"
                style="margin-left: 20px"
                >{{ data1.device_type }}</text
              >
              <view class="uni-grow"></view>
            </view>
            <view class="desc">
              <text class="color-text-desc">{{ data1.device_funciton }}</text>
            </view>
          </view>
        </view>
        <view
          ...
          >
            <view class="item-content-base">
              ...
              <text
                class="color-text color-text-num"
                style="margin-left: 20px"
                >{{ data2.device_type }}</text
              >
              <view class="uni-grow"></view>
            </view>
            <view class="desc">
              <text class="color-text-desc">{{ data2.device_funciton }}</text>
            </view>
          </view>
        </view>
      </view>
...
import asepService from "../../service/hra/asep";
...
export default {
  data() {
    return {
      ...
      scrollHeight: 500,
      scrollTop: 0,
      newAsepId: asepService.genGuid(),
      newTherpId: therpService.genGuid(),
    };
  },
  onLoad() {
    ...
    uni.getStorage({
      ...
      success: (val) => {
        if (val.data) {
          ...
          this.loadParam = { guid: this.newAsepId };

          asepService.query({ foreign_id: this.data2.guid }).then((data) => {
            if (data.length) {
              this.asepData = data[0];
              if (this.asepData.related_value) {
                this.value = this.asepData.related_value * 1;
              }
              this.loadParam = { guid: this.asepData.guid };
            }
          });
  ...
  methods: {
    showTabByData() {
      if (this.data2.correlation_type != "asep") {
        this.tabClick(this.tabs[1]);
      }
    },
    pageClick() {},
    tabClick(tab) {
      this.service = tab.index ? therpService : asepService;
      let data = tab.index ? this.therpData : this.asepData;
      this.tabIndex = tab.index;
      this.value = data.related_value * 1;
      let newId = tab.index ? this.newTherpId : this.newAsepId;
    ...
    onFlowChange(flowData) {
      let data = this.tabIndex ? this.therpData : this.asepData;
      let fn = data.guid ? "update" : "insert";
      if (fn == "insert") {
        data.guid = this.tabIndex ? this.newTherpId : this.newAsepId;
    ...
    save() {
      ...
      if (this.tabIndex) {
        ...
        updateData.correlation_type = "asep";
      }
      typeACService.update(updateData).then(() => {
        this.back();
        uni.showToast({
          title: "保存成功!",
          duration: 2000,
