        ...
        >
          ...
          <view class="uni-column uni-grow" style="padding: 10px 0 10px 20px">
            <view class="list-item-content" @click="viewData(data)">
              <text>{{ data.human_error_code }}</text>
              <text class="color-text color-text-num">{{ data.system }}</text>
              <view class="uni-grow"></view>
            </view>
            <view class="desc">
              <text class="color-text-desc">{{ data.accident }}</text>
            </view>
          </view>
          <view
            class="link-btn"
...
export default {
  ...
  methods: {
    ...
    loadData() {
      typeCCService.query({ group_id: this.group_id }).then((datas) => {
        this.datas = datas;
      });
    },
    save() {
      ...
      this.validName().then(() => {
        if (this.mode == "create") {
          let datas = [];
          let group_id = typeCCService.genGuid();
          this.datas.forEach((d) => {
            datas.push({
              guid: typeCCService.genGuid(),
              task_id: this.task_id,
              event_id: d.guid,
              group_id: group_id,
              group_name: this.group_name,
              human_error_code: d.human_error_code,
              accident: d.accident,
              system: d.system,
              accident_desc: d.accident_desc,
          ...
          typeCCService.updateName(this.group_id, group_name).then(() => {
            uni.showToast({
              title: "保存成功!",
              duration: 2000,
            });
    ...
    toAddEvent() {
      ...
      uni.navigateTo({
        url: "/pages/hra/selectEvent?type=C",
      });
    },
    viewData(data) {
      if (this.mode == "update") {
